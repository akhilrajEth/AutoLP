...
export function calculateImpermanentLoss(
  ...
  const depositValue = token0Amount * depositPrices.token0Price + token1Amount * depositPrices.token1Price
  const currentValue = token0Amount * currentPrices.token0Price + token1Amount * currentPrices.token1Price
  ...
  return {
    ...
    token0Change: ((currentPrices.token0Price - depositPrices.token0Price) / depositPrices.token0Price) * 100,
    token1Change: ((currentPrices.token1Price - depositPrices.token1Price) / depositPrices.token1Price) * 100,
  }
}
