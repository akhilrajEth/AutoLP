    ...
    <div className="card animate-fade-in">
      ...
      <div className="space-y-6">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <p className="font-medium text-gray-900">
                    {tokenAmounts.ethAmount} ETH
                  </p>
                  <p className="text-sm text-gray-500">
                    {formatCurrency(tokenAmounts.ethAmountUSD)}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <p className="font-medium text-gray-900">
                    {tokenAmounts.usdcAmount} USDC
                  </p>
                  <p className="text-sm text-gray-500">
                    {formatCurrency(tokenAmounts.usdcAmountUSD)}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="text-sm text-blue-800">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    <span className="text-blue-600">ETH Price:</span>{" "}
                    {formatCurrency(poolInfo.token0Price)}
                  </div>
                  <div>
                    <span className="text-blue-600">USDC Price:</span>{" "}
                    {formatCurrency(poolInfo.token1Price)}
                  </div>
                  <div>
                    <span className="text-blue-600">Total Liquidity:</span>{" "}
                    {formatNumber(parseFloat(poolInfo.liquidity) / 1e18, 0)} ETH
                  </div>
                  <div>
                    <span className="text-blue-600">Price Impact:</span>{" "}
                    {formatNumber(tokenAmounts?.priceImpact || 0, 3)}%
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="text-xs text-gray-500 text-center">
          By adding liquidity, you agree to the terms of service and acknowledge
